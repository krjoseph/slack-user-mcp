...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, context) => {
    ...
    try {
      ...
      const authToken =
        token ?? extractAuthToken(context?.requestInfo?.headers?.authorization);
      if (!authToken) {
        throw new Error(
          'No auth token provided. Please set SLACK_TOKEN (or SLACK_BOT_TOKEN) environment variable or provide an auth token in the request headers.'
        );
      }
